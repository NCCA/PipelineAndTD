    ...
    def doIt(self, args):
        ...
        if parser.isFlagSet("-h"):
            OpenMaya.MGlobal.displayInfo(
                "This command creates a number of spheres in the scene"
            )
            ...
            OpenMaya.MGlobal.displayInfo(
                "-mr -minRadius: The minimum radius of the sphere"
            )
            OpenMaya.MGlobal.displayInfo(
                "-mm -maxRadius: The maximum radius of the sphere"
            )
            return True
...
def initializePlugin(plugin):
    ...
    try:
        plugin_fn.registerCommand(
            CustomSphereSolution.CMD_NAME,
            CustomSphereSolution.creator,
            CustomSphereSolution.newSyntax,
        )
    except:
        OpenMaya.MGlobal.displayError(
            f"Failed to register command: {CustomSphereSolution.CMD_NAME}"
        )
...
def uninitializePlugin(plugin):
    ...
    try:
        ...
        OpenMaya.MGlobal.displayError(
            f"Failed to deregister command: {CustomSphereSolution.CMD_NAME}"
        )
